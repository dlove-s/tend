...
export function setupCard(feedId: string, kind: "inbox" | "company"): Card {
  ...
  if (kind === "inbox") {
    return {
      ...
      eyebrow: "Feed setup",
      title: "Arm a Claude session for Inbox, then collect its first sweep.",
      why: "Tend is built for a Claude session's in-app browser preview. Inbox needs one Claude session armed before it can operate the configured Gmail recipe.",
      blocks: [
        { id: "brief", type: "rich_text", label: "How it works", text: "Claude inspects new Gmail threads, decides disposition before drafting, and preserves an exact approval gate before any send." },
        { id: "checklist", type: "checklist", label: "First run", items: ["Keep this feed open in a Claude session's in-app browser preview", "Run tend setup claude --feed inbox and paste the prompt into that session, or run /tend to arm it directly", "Let the session bind its Claude lane and start the wake monitor", "Open or wake that session and say “go deal with the feed”", "Review the first real cards and correct the framing"] },
      ],
  ...
  return {
    ...
    blocks: [
      ...
      { id: "clarify", type: "clarification", label: "One instruction is enough", text: "Tell Claude which sources to include or exclude. It will update the recipe and show the result here." },
    ],
